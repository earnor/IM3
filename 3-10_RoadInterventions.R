...
data <- read.csv("3-10_RoadInterventions_csvData.csv"
                 ,header=TRUE,sep=";",dec=",") 
attach(data)
...
dev.copy(png,'E3-10_eta.png',width=2000,height=1330)
dev.off()
...
plotetaT <- ggplot(data=rdftidyB,aes(x=T,y=value
  ...
  scale_x_continuous(breaks = seq(0, max(rdftidy$T), by = 1)) +
  xlab("T [years]") + 
  ylab("Average cost per year [mu/yr]") + # Set axis labels
  ylim(0,200) +
  theme_bw(base_size=36) +
...
{
  optimal[n,1] <- key[n]
  optimal[n,2] <- data$T[which.max(data[,n+1])]
...
dev.copy(png,'E3-10_etaB.png',width=2000,height=1330)
dev.off()
