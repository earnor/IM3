# *-----------------------------------------------------------------
# | PROBLEM NUMBER: 4.6
# | PROBLEM NAME: Tunnel fans
# | UPDATE: AE     
# | DESCRIPTION: In this program, we go through the steps of 
# |              calculating the mean costs over tu using the Semi-
# |              Markov method.
# | 
# | KEYWORDS: Semi-Markov, failure, fan, system, steady-state, 
...
# We are given that no fan lasts longer than N years, according to 
# the given probabilities. If one fan fails, and the other is older
# than the critical N.cr years, we replace both, but if it is 
# younger, only the failed one is replaced.Replacing one costs C.1, 
# but replacing both C.2 monetary units.

# The transition probibilities need to be calculated using 
# conditional failure probabilities dependent on the age of the fan. 
# The probabilities when fans' age is zero are given and defined by 
# p0.a and p0.b for fans a and b.

# Further on the process of the program:

# The failure probabilities are also used to calculate the 
# probabilities of leaving a particular state, which is a 
# representation of the share of time spent in that state. 
# This allows a calculation of a mean transition time, v.ij,
# i.e. given that an item is in a particular state, how long will it 
# stay in that state. 
...
N.cr  <- 2    # critical age
N     <- 7    # Maximum age of fan
C.1   <- 1000 # costs if only one fan is replaced
C.2   <- 1400 # costs if both fans are replaced.

# vector of possible values for t, used to calculate conditional
# probabilities
T <- seq(0,N.cr,by=1) 

# Conditional probability of failure in year i, at t=0
p0.a <- c(  0.08
          , 0.14
          , 0.17
          , 0.17
...
# The possible states the system can have is defined as characters 
# in states. We first define it as a vector of length 2*N.cr+1. The 
# relative state of eachfan to the other is then collected in the 
# matrices a and b
...
# P is a matrix collecting all values for probabilities of 
# leaving a state.
P       <- array(dim=c(N,length(states)))

# X multiplies the probability i,j multiplied by the respective 
# time. The sum  of this over all time values then equals the mean
...
# Cost per tu, these matrices collect numerator and denominator for
# all ij
...
# Step 3: Using calculated transition matrix, we find steady state
# probabilities
...
#Step 4: Estimating the probability of system in a state failing in 
# a certain year
...
for (j in 1:length(states)) 
{
  for (i in 1:N)
  {
    ...
    if (j==1)
    {
      ...
      {
        # This is never the case for j=1, but keep consistent
        P[i,j] <- 0
      }     
    }
    else if (j==2 || j==3) 
...
## Step 5: Find mean transition time and average cost of transition
...
## Step 6: calculate the mean cost per units of time
