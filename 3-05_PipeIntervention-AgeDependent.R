...
# |              in terms of optimisation of availability based on
# |              Problem 3.4. A Weibull distribution is used to 
# |              model the probability of failure. 
# | 
...
# From problem 3.4, we know the following:
...
#For problem 3.5
t.CI <- 10 # Down-time during a corrective intervention
t.PI <- 1  # Down-time during a preventive intervention
...
tau <- matrix(nrow=T.max) # availability
...
for (T in 1:T.max){
  ...
  tau[T] <- E[T]/(E[T]+F[T]*t.CI+S[T]*t.PI) # operational time per tu
...
result=data.frame(F,S,E,eta,tau)
...
cat('Time at which yielding the maximum availability time \n')
print(which.max(tau))
cat('Maximum availability per unit time \n')
print(max(tau))
...
plotres <- data.frame(1:T.max,tau)
names(plotres) <- c("T","tau")

plottau <- ggplot(data=plotres,aes(x=T,y=tau)) + 
...
data <- plotres
optimal[1,1] <- data$T[which.max(data$tau)]
optimal[1,2] <- max(data$tau)
optlines <- data.frame(key = c("z","z")
                       ...
                       , y1 = c(optimal$y,0.9)) # Same as y.min
...
plottau + geom_point(data=optimal
                     ...
                     , colour="navyblue"
                     , size=6) +
  annotate("text", label="Optimal - 0.9965\nT=428"
           , x=optimal[1,1]+80, y=optimal[1,2]-0.01
           , size=12) +
...
dev.copy(png,'E3-05_tau.png',width=2000,height=1330)
dev.off()
### ------------------PLOT END-------------------
